...
export METASTORE_DB_HOSTNAME=${METASTORE_DB_HOSTNAME}
...
if [ "${METASTORE_TYPE}" = "mysql" ]; then
  ...
  /opt/apache-hive-metastore-3.0.0-bin/bin/schematool -initSchema -dbType mysql -userName ${MYSQL_USER} -passWord ${MYSQL_PASSWORD}
